...
- **交互式新手教程**:首次登录自动开始,24 步覆盖全部功能。聚光灯高亮真实按钮(四周压暗),导航类步骤必须点击那个真实按钮才前进(搜索入口 → 附近 → 任务卡 → 发布 ➕ → 发布求助 → 消息 → 我 → 设置齿轮 → 我的日历……),信息类步骤用「继续」;覆盖搜索高级筛选、任务详情、发布、消息、主页装扮、日历、积分、圈子活动、安全中心、Plus、Pro、任务加速、机构版与管理员后台。随时可跳过,可从「更多 → 新手教程」或个人主页设置菜单重看。
...
- **会员订阅**(更多菜单 → 会员订阅):免费 / Plus / Pro 三档并列的订阅页(Claude/GPT 式),Pro 为 Plus 的进阶版,包含全部 Plus 权益 + 专业工具。会员标识为独立的文字胶囊(Plus 暖红 / Pro 墨色),与认证体系完全分开;页面明确列出「Plus 不会带来的东西」与「Pro 不能做的事」。免费用户保有完整互助闭环。
- **会员功能全部实装**,订阅页每张权益卡标注实际入口:高级筛选与保存搜索(搜索结果页)、命中保存条件的新任务即时通知、任务模板与草稿/预约(发布页)、曝光与申请数据(推广效果页)、主页装扮签名色(个人主页)、多社区管理(圈子页)、Pro 专业主页编辑与数据分析(订阅页工作台)、Pro 私信自动回复、Pro 活动报名名单与签到(活动笔记详情)。
- **内置日历(免费)**:更多菜单 → 我的日历,月视图汇总互助安排与圈子活动,支持整体导出 .ics;进行中任务详情也可单独「添加到日历」。日历属于基础能力,对所有用户免费。
...
- **机构版 / 品牌公益赞助 / 验证服务**:B2B2C 机构授权、明示赞助方的公益活动、按第三方成本收费的验证项目(基础身份认证永远免费)。
